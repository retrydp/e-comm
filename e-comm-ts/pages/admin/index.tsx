...
const AdminPannel = () => {
   ...
   */
  const additionalColors = (input: HTMLInputElement): void => {
    const arrayFromValues: string[] = input.value.split(',').filter(Boolean);
    const inputs: JSX.Element[] = arrayFromValues.map((value: string) => (
      <div className="body__row">
        <label htmlFor={value} className="body__input-label" style={{ color: value }}>
          {value}
        </label>
        <input id={value} type="text" className="body__input" placeholder="image for color" required />
      </div>
    ));
        ...
        <form className="body__wrapper">
          ...
          <div className="body__row">
            ...
            <input
              ...
              required
              onChange={(event: React.ChangeEvent<HTMLInputElement>): void => additionalColors(event.target)}
            />
          </div>
          ...
          <div className="body__row">
            ...
            <textarea id="description" rows={4} className="body__input" placeholder="Description" style={{ resize: 'vertical' }} />
          </div>
